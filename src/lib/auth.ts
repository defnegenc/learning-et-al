    ...
    async jwt({ token, user, account }) {
      ...
      if (user && account) {
        ...
        if (!dbUser) {
          ...
          dbUser = { id, email: user.email!, name: user.name ?? null, image: user.image ?? null, createdAt: new Date(), timezone: "America/New_York", contentMix: 50, cadence: "daily" as const, emailVerified: null, emailOptOut: false, digestPaused: false };
        }
