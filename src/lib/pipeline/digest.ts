...
Return JSON (no markdown fences):
{
  ...
  // One visible edition per reader per day, backed by the
  // digests_user_date_visible_unique partial index. A force regen retires the
  // current visible edition into hidden history and the fresh one takes the
  // slot — before this, regen inserted a second visible edition for the same
  // date, which is where the duplicate-edition mornings came from.
  let retiredForRegen: string[] = [];
  if (force) {
    const visibleToday = await db
      .select({ id: digests.id })
      .from(digests)
      .where(and(eq(digests.userId, userId), eq(digests.date, today), or(isNull(digests.hidden), eq(digests.hidden, false))));
    retiredForRegen = visibleToday.map(row => row.id);
    if (retiredForRegen.length > 0) {
      await db.update(digests).set({ hidden: true }).where(inArray(digests.id, retiredForRegen));
    }
  }

  let digest: typeof digests.$inferSelect;
  try {
    [digest] = await db.insert(digests).values({
    userId, date: today,
    theme: finalTheme,
    ...
    // Lost the race to a concurrent run between the early existing-check and
    // this insert: its edition is the edition of record for today, so return
    // it rather than force a duplicate past the unique index. (Cron path only
    // — a force regen retires the visible edition first, so a conflict there
    // is a real error worth surfacing.)
    const err = error as { code?: string; message?: string };
    const isUniqueViolation = err.code === "SQLITE_CONSTRAINT_UNIQUE" || /unique constraint failed/i.test(err.message || "");
    if (!force && isUniqueViolation) {
      const winner = await db.query.digests.findFirst({
        where: and(eq(digests.userId, userId), eq(digests.date, today)),
        orderBy: desc(digests.createdAt),
      });
      if (winner) {
        console.log(`[Digest] Edition for ${today} was generated by a concurrent run; returning it instead of publishing a duplicate.`);
        return winner;
      }
    }
    if (retiredForRegen.length > 0) {
      await db.update(digests).set({ hidden: false }).where(inArray(digests.id, retiredForRegen));
    }
    throw error;
  }
