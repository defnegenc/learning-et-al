import { BANNED_WORDS_RULE, PROMPT_ONLY_BANNED_RULE } from "./banned-words";
 ...
 */
export const CLAIM_SELF_CHECK = `CLAIM SELF-CHECK - run before returning, for every gist, takeaway, hook, stat, line, and claim:
1. Point to the exact source sentence that supports it. If you cannot, cut the claim.
2. If your wording upgrades the source's hedge ("tend to" became "everywhere", "suggests" became "proves"), put the hedge back.
3. If it names a construct the source never measured (the source measured performance and you wrote "trust"), rename it to the measured construct or cut it.
4. If it describes an experiment, comparison, or result the source did not run or report, remove it - never sketch a study design the paper did not run.`;

export const EVIDENCE_RULES = `EVIDENCE FIDELITY - four grounding rules that outrank style:
...
- HEDGE FIDELITY: keep the source's hedges and its constructs. "Tend to", "suggests", and "in this sample" stay hedged - never "everywhere", "always", "almost identically", or "across cultures" unless the source says exactly that. Name only the constructs the source actually measured: if it measured performance or preference, never call the result "trust", "belief", or "safety", and never describe an experiment the source did not run.`;
...
5. SCOPE UPGRADE: a hedged or bounded source claim written as universal ("everywhere", "always", "no one can", "the field is shrinking") when the abstract does not say that. This includes absolute claims about what people notice, point to, or check ("nobody's consciously checking", "not any flaw readers could point to") - if no abstract reports what readers or viewers noticed, any claim about it is unsupported.
6. DOMAIN TRANSFER: a claim about a domain, medium, population, or stimulus no source studied. A study of AI-labeled news articles does not support a claim about feelings toward AI-made visual art; a study of text does not support a claim about images. The gist and the synthesis verdict must stay inside the domains the sources actually measured. An opening verdict that answers the theme question at a broader scope than the sources ("Mostly, yes" when the sources cover only one corner of the question) is a flag even when each narrower claim is supported.
7. STATED AS OBSERVED: stated preferences, hypothetical choices, intentions, or self-reports written as observed behavior or market outcomes. A choice-based conjoint or a willingness-to-pay measure does not show that people "pay less", "buy less", or "spend less" - the supported wording is "would pay less" or "value less". Flag any payment, purchase, or behavior verb the study did not directly observe.
