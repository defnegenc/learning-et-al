...
| Reading detail | 680px column: title, byline, the five beats, `Remember this`, what's happened since. Nothing else. Hard words are the dotted rule, never a fill. Fill means a passage you chose: ink `::selection` while the mouse is down, the paper's own hue on release, marked in place with a numeral after it. One verb, "Ask"; an empty field asks "What does this mean?". **Answers never enter the column** — each is a card in the rail, headed by its passage in the paper's hue. Glossary is a menu in the top right; asking anything else is a square in the bottom corner |
