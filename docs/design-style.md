...
| Reading detail | 680px column. Title, byline, gist, then what's happened since. Hard words are the dotted rule, never a fill. Fill means a passage you chose: ink `::selection` while the mouse is down, the paper's own hue the moment it is released, marked in place in the paragraph with a numeral after it. One verb, "Ask", and an empty field asks "What does this mean?". **Answers never enter the column** — each is a card in the rail, headed by its passage in the paper's hue, above the glossary and Ask. The rail is one sticky scroll region, not three |
