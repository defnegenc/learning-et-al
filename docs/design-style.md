...
| Reading detail | One centred 720px column at every width: title, byline, the five beats, `Remember this`, a `Glossary` handle, what's happened since. There is no "Ask this paper" section: every question here is about a passage. The words the reader kept live in a 380px drawer off the right edge, reached from the top bar or from the foot of the read. Hard words are the dotted rule, never a fill. Fill means a passage you chose: ink `::selection` while the mouse is down, the paper's own hue on release. One verb, "Ask"; an empty field on a held passage asks "What does this mean?". An answer is **an ink square with a number in it** at the end of the passage, opening into a margin that does not exist until something opens into it (below 1220px it opens in the flow instead). There is exactly **one text bar on the page**, it moves to whatever you last highlighted or tapped, and its resting state is no bar at all. A reader who has never highlighted gets one `TipStrip` above the gist saying so |
