...
| `vault/reading-paper-detail.tsx` | The reading view — the body of `/library/[paperId]` | ✅ a centred 720px column: title, byline, five selectable beats, the `Remember this` frame, `AskSection`, `Glossary`, the citing work. The product does not explain itself in it (no "pulled in for", no "you rated yourself"). Open an answer above 1080px and the page widens to 1140 with a 380px margin beside the read; close the last one and the margin is gone. Takes `index` for the paper's hue, and a `fixture` seeds state synchronously so the prototype renders the real thing rather than its loading state |
...
| `GlossaryMenu` / `HomeworkRow` (internal to `reading-paper-detail.tsx`) | Every hard word, in the top right of the reading view's own bar, which is sticky under the site header so it travels with the reader; and the citing works as one hairline-separated row per paper with a save on the right | ✅ closes on Escape or a click away. Cards were tried for the citing works and reverted: too much object for a list of things you have not read |
