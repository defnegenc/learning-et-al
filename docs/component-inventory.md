...
| `SelectionMenu` / `DigPanel` / `DigThisBeat` (internal to `reading-paper-detail.tsx`) | Highlight to dig deeper: the floating menu (a question box, then **Dig deeper**), the collapsible aside the streamed answer lands in under its beat, and the per-beat touch affordance | ✅ the selection is the one sanctioned acid fill (`SELECTION_FILL`); a dug passage stays marked in the prose in the paper's hue via `annotateBeat`, and the aside is an indent behind one 2px rule, not a box |
