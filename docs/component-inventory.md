...
| `SelectionMenu` / `DigThisBeat` (internal to `reading-paper-detail.tsx`) | Highlight to ask: the floating bar and the per-beat touch affordance. The bar is one field and **Ask**, plus **+ Glossary** when (and only when) the highlight is term-shaped: under 60 characters, five words or fewer, no internal sentence punctuation (`looksLikeTerm`) | ✅ the drag is the ordinary ink `::selection` and the passage turns the paper's hue on release, drawn by `annotateBeat`, with a `user-select: none` numeral after it. Not a second "Ask" / "Dig deeper": a word is a thing you keep, a passage is a thing you ask about |
| `Conversation` (internal to `reading-paper-detail.tsx`) | The rail, 420px, full viewport height: header, scrolling log, composer that never moves. **One thread of talk holding everything** — highlighted passages and typed questions, in the order they happened. A passage-anchored block wears its passage in the paper's hue with its numeral, and pressing it scrolls the read back to the sentence; clicking the sentence scrolls here and lights the block. Typing continues the conversation rather than starting a new one | ✅ replaces `DigCard` + `AskCompanion` + `AskThread`, which were two places to look for one conversation, split by whether a question happened to start from a highlight |
