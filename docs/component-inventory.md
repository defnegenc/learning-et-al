...
| `Glossary` / `HomeworkCard` (internal to `reading-paper-detail.tsx`) | The folded word list at the foot of the read, and the citing works as `PaperCard` compact in a two-up grid. `HomeworkCard` shapes an OpenAlex work into a `PaperItem` and puts its save in the card's `footnote`, because a citing work has no row yet and saves through `save-external` | ✅ one card for a paper. `AskSection` is deleted: every question on this page is about a passage now |
