...
| `TalkSheet` (internal to `reading-paper-detail.tsx`) | Everything that is not the paper: one sheet docked to the foot of the viewport, the width of the column, identical on phone and laptop. Folded it is a bar; open it is a body and a composer at most 68vh tall. The body is the conversation or the glossary, chosen with `Segmented`. Held passages sit above the field with **+ Glossary** and a × | ✅ replaces `Conversation` + `Glossary` + `DigCard` + `AskCompanion` + `AskThread` + `SelectionMenu`. There is no rail: it was the mobile bug, not a media query |
