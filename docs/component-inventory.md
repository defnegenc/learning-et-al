...
| `TipStrip` | The one tip: bordered band, no shadow, bolded sentence-case "Tip:" lead-in, one Body line, a × that retires it. The save tip above the digest and the highlight tip in the reading view are both this | ✅ new — a surface that teaches something once uses this rather than inventing a second shape |
...
| `vault/reading-paper-detail.tsx` | The reading view — the body of `/library/[paperId]` | ✅ a centred 720px column: title, byline, five selectable beats, the `Remember this` frame, a `Glossary` handle, the citing work. The product does not explain itself in it (no "pulled in for", no "you rated yourself"). Open an answer above 1080px and the page widens to 1140 with a 380px margin beside the read; close the last one and the margin is gone. Takes `index` for the paper's hue, and a `fixture` seeds state synchronously so the prototype renders the real thing rather than its loading state |
| `useSelectionPick` / `DigThisBeat` (internal to `reading-paper-detail.tsx`) | Highlight to ask. **Nothing floats over the selection**: the hook reports the passage and `HeldComposer` opens on it. `DigThisBeat` is the same gesture for touch, per beat | ✅ the drag is the ordinary ink `::selection` and the passage turns the paper's hue on release, drawn by `annotateBeat`. `beatFor` + `clipToBeat` make a multi-line drag, or one that runs off the end of a paragraph, resolve to a real beat |
| `AnswerSquare` / `AnswerPanel` / `HeldComposer` (internal to `reading-paper-detail.tsx`) | A question tagged onto its passage. The **square** (an ink box with a number) sits at the end of the coloured passage like a footnote marker and is the resting state; the **panel** is the answer, and `HeldComposer` is the field that precedes it. Both open in the margin beside the passage above 1080px and in the flow beneath it below, via `useMarginTops` | ✅ replaces `TalkSheet` + `Conversation` + `DigCard` + `AskCompanion` + `AskThread` + `SelectionMenu` + `DigPanel`. Nothing fixed, floating, sticky or docked; the margin itself does not exist until something opens into it |
| `GlossaryDrawer` / `HomeworkCard` (internal to `reading-paper-detail.tsx`) | The word list, in a 380px drawer off the right edge, and the citing works as `PaperCard` compact in a two-up grid. Two handles reach the drawer, `Glossary (n)` in the top bar and the same count as a text link at the foot, because the top bar has scrolled away by the time anyone finishes the read. Keeping a word opens it. `HomeworkCard` shapes an OpenAlex work into a `PaperItem` and puts its save in the card's `footnote`, because a citing work has no row yet and saves through `save-external` | ✅ one card for a paper. `AskSection` is deleted: every question on this page is about a passage now. The drawer is the one thing on this page that covers it, and only while it is open |
