...
| `vault/reading-paper-detail.tsx` | The reading view — the body of `/library/[paperId]` | ✅ 1240px two-column, a page rather than a portal overlay. Left: title, byline, five companion beats each selectable, the `Remember this` card frame, the citing work. **Nothing an answer produces lands in that column**, and neither does the product explaining itself: no "pulled in for" line, no "you rated yourself" callout. Right: a sticky, self-scrolling rail holding the `DigCard` stack and nothing else. Top bar: Back, then `GlossaryMenu` and `Read the full paper`. `AskCompanion` floats in the bottom corner. Takes `index` for the paper's hue, and a `fixture` seeds state synchronously so the prototype renders the real thing rather than its loading state |
| `SelectionMenu` / `DigCard` / `DigThisBeat` (internal to `reading-paper-detail.tsx`) | Highlight to ask: the floating bar (one field, one **Ask**), the answer card in the rail, and the per-beat touch affordance | ✅ the drag is the ordinary ink `::selection` and the passage turns the paper's hue on release, drawn by `annotateBeat`. A card is the full frame plus the one shadow, **headed by its passage in the paper's hue**, tied to a numeral in the prose that runs both ways: hover either and the other responds, click either to be taken to the other. Replaces `DigPanel`, which was an indent under the beat and cut the read |
| `AskCompanion` (internal to `reading-paper-detail.tsx`) | Ask this paper, for questions that are not about one sentence: a 52px square in the bottom corner on every paper at every scroll position, unfolding into a 380px panel with the thread, the three suggested questions and a composer | ✅ replaces the rail panel `AskThread`. Same frame, same shadow, same wash header; the fullscreen toggle and the quoted-passage composer are gone, because a passage now becomes a card instead |
| `GlossaryMenu` (internal to `reading-paper-detail.tsx`) | Every hard word, in the top right next to `Read the full paper`. Closed by default; closes on Escape or a click away | ✅ replaces the rail panel `Glossary`. A reference list should not sit in the same column as the answers the reader made themselves |
