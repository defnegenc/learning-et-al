...
| `vault/reading-paper-detail.tsx` | The reading view — the body of `/library/[paperId]` | ✅ **one centred 720px column at every width**: title, byline, five selectable beats, the `Remember this` frame, the citing work. Nothing else is in the column, and the product does not explain itself in it (no "pulled in for", no "you rated yourself"). `TalkSheet` docks at the foot of the viewport. Takes `index` for the paper's hue, and a `fixture` seeds state synchronously so the prototype renders the real thing rather than its loading state |
