...
| `AnswerTag` / `HeldComposer` (internal to `reading-paper-detail.tsx`) | A question, tagged onto the passage it came from. `HeldComposer` is the field that opens in the flow under a fresh highlight (with **+ Glossary** when the highlight is term-shaped); `AnswerTag` is the answer that replaces it, and the **ink square with a number** it collapses to, sitting at the end of the passage like a footnote marker | ✅ replaces `TalkSheet` + `Conversation` + `DigCard` + `AskCompanion` + `AskThread` + `SelectionMenu` + `DigPanel`. Nothing fixed, floating, sticky or docked; beats are `div`s so a block can open inside a sentence |
