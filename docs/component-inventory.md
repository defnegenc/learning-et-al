...
| `AskSection` / `Glossary` (internal to `reading-paper-detail.tsx`) | The two things with nowhere else to be, as sections at the foot of the read: a field for questions that are not about one sentence, and the folded word list | ✅ in the column, in flow, not panels over the page |
