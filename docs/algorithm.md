...
- **Field-sensitive recency window**: the judge tier classifies the seeded field's volatility - volatile areas search the current year plus the prior two years, evergreen areas (art, history, perception, philosophy and other fields where the strongest source is often decades old) search a ten-year window with no recency pressure at all, and everything else searches the current year plus the prior five years. Obvious volatile vocabulary short-circuits the classifier; a classifier failure falls back to the standard window rather than guessing.
...
- If every OpenAlex scope returns zero, Semantic Scholar receives the deterministic field stored on the seeded user interest; arXiv remains the final fallback. No LLM-generated label controls retrieval. arXiv results carry the year parsed from the feed's `<published>` tag (0 when absent, the OpenAlex convention); they used to be stamped with the current year, which handed every arXiv paper the maximum recency bonus and let an old one satisfy the current-year floor.
