...
**Stage A: Metadata** (AI call 8, judge tier, fires **concurrently with Stage B**) — per-paper summaries, keywords, findings, connectionToTheme, **plainName** (plain-language paper name shown on cards above the academic title), **takeaway** (`hook` = the one surprise, `stat` = concrete anchor or null, `line` = a distinct conversational implication — powers Conversational Papers; hook/stat/line may not paraphrase one another), **methodType/methodFacts/claim** (what the source IS — "Field study", "Opinion piece", "News feature" — plus 2-3 short how-they-did-it facts and the one-sentence central claim; these fill the card's themed See-more tiles), keyConcepts, suggestedQuestions. `plainName` must distinguish the source rather than restate the digest headline or takeaway. Scripted-casual filler ("So you'd think", "Turns out", "It's kind of like", "which sounds obvious") is explicitly banned; clarity and evidence supply the voice. Uses `metadataPrompt`. keyConcepts now aggressively captures jargon a non-expert trips on — model/system names (RoBERTa, DistilBERT), technical methods (subword tokenization, self-attention), and acronyms (EEG, NLP) — so the synthesis hover-definitions actually fire on scary words.
  ...
  *Stage A is the load-bearing call and it is allowed to fail loudly (2026-08-25).* Everything on a paper card except the title and byline comes from here, and the digest view **replaces a source's synthesis paragraph with its card**, so a paper with no metadata renders as its own raw abstract with nothing around it: no plain-language name, no hero line, no Findings, no Takeaway. Three of those in a row and the digest is three pasted abstracts, which is exactly what shipped. Three things changed. (1) **The output ceiling is sized to the paper count** (`min(12000, 4000 + 1200 × papers)` instead of the global 4096). Stage A emits a plain name, a summary, three findings, a takeaway triple, method facts and a claim for *every* paper plus the glossary, in one object, and on a thinking-by-default judge model the reasoning is spent from the same budget before the first character of the answer. (2) **`extractJson` salvages a truncated response** rather than returning `null`: it cuts at the last complete `}`/`]` and closes the open containers, so a run that dies in paper 4 still delivers papers 1-3 instead of nothing. Only a closing bracket counts as a cut point, so a half-written object is dropped whole. (3) **A shortfall is retried once and logged at error level.** The old path had one `console.log` and empty defaults. `aiChat` also warns whenever a response comes back with `finish_reason: "length"`, which is a 200 with a partial body and was previously invisible.
...
| 8 | Metadata (Stage A) | 6 | Always | judge | ~6000 | ~2000 (ceiling `4000 + 1200/paper`) |
