...
Every digest is built around a single **central question** with wow factor (max 8 words).
...
6. **Theme revision**: After papers are found, the LLM revises the central question to better thread the actual papers together. This keeps the theme grounded in what was found.
...
- **Brutalist research archive aesthetic**: hard borders, box shadows, uppercase mono labels, crosshair cursor, accent colors only in tags
...
- Log what worked AND what didn't work in the Mistakes Log
- Update Feature Log with dates for every new feature
