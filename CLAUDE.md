...
- `FACTCHECK_AI_PROVIDER` / `FACTCHECK_AI_KEY` / `FACTCHECK_AI_MODEL` / `FACTCHECK_AI_BASE_URL` — Optional. An independent, **cross-family** fact-checker for the digest synthesis: a model from a DIFFERENT provider (e.g. Gemini Flash under a Claude pipeline) reads the Stage C draft against the raw source texts and its issues merge into the existing Stage D revision. Activates only when provider AND key are both set; never blocks a digest; unset = pass never runs. This is deliberately a separate provider+key from `CRON_AI_*` — the point is an unshared failure mode (see `factCheckConfig()` in `src/lib/ai/provider.ts` and `docs/plans/cross-provider-judge.md`).
